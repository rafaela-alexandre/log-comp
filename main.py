...
class Code:
    instructions      = []  # código do _start
    func_instructions = []  # código das funções
    in_func           = False
    ...
    def dump(filename: str) -> None:
        ...
        with open(filename, 'w') as f:
            ...
            f.write('  push ebp\n')
            f.write('  mov ebp, esp\n\n')
            f.write('  ; aqui comeca o codigo gerado:\n\n')
...
class Identifier(Node):
    ...
    def generate(self, st):
        ...
        if var.shift < 0:
            # parâmetro: [EBP + |shift|]
            Code.append(f"  mov eax, [ebp+{-var.shift}] ; param {self.value}")
        else:
...
class VarDec(Node):
    ...
    def evaluate(self, st):
        ...
        if len(self.children) > 1:
            ...
            if result.vartype != self.value:
                raise Exception(f"[Semantic] Type mismatch in declaration of '{name}'")
    ...
    def generate(self, st):
        ...
        st.create_variable(name, self.value, is_func=False)
        var = st.table[name]
        Code.append(f"  sub esp, 4 ; var {name} [EBP-{var.shift}]")
        if len(self.children) > 1:
            self.children[1].generate(st)
...
class Return(Node):
    ...
    def generate(self, st):
        # gera o valor de retorno em EAX, restaura frame e retorna
        self.children[0].generate(st)
        Code.append("  mov esp, ebp ; restaura pilha")
        Code.append("  pop ebp")
        Code.append("  ret")
...
class FuncDec(Node):
    """
    value        = return type (str or None)
    children[0]  = Identifier (nome)
    children[1:-1] = VarDec (params)
    ...
    def generate(self, st):
        func_name  = self.children[0].value
        param_nodes = self.children[1:-1]
        body        = self.children[-1]

        # registra a função na symbol table global
        ret_type = self.value if self.value else "void"
        st.create_variable(func_name, ret_type, is_func=True)
        st.set_value(func_name, self)

        # gera código da função na seção de funções
        Code.in_func = True
        Code.append(f"{func_name}:")
        Code.append("  push ebp")
        Code.append("  mov ebp, esp")

        # cria symbol table local para os parâmetros
        # parâmetros ficam em [EBP+8], [EBP+12], etc. (EBP+4 = endereço de retorno)
        func_st = SymbolTable(parent=st)
        for i, param in enumerate(param_nodes):
            param_name = param.children[0].value
            param_type = param.value
            offset = 8 + i * 4  # [EBP+8], [EBP+12], ...
            # shift negativo não serve — usamos offset positivo para args
            var = Variable(param_type, None, 0, False)
            var.shift = -(offset)  # truque: armazenamos negativo para diferenciar de locais
            func_st.table[param_name] = var

        # gera corpo
        body.generate(func_st)

        # epilogo padrão (caso não haja return explícito)
...
class FuncCall(Node):
    ...
    def evaluate(self, st):
        ...
        if ret_type and ret_type != "void":
            if result is None:
                raise Exception(f"[Semantic] Function '{func_name}' must return a value")
            if result.vartype != ret_type:
                raise Exception(f"[Semantic] Function '{func_name}' must return {ret_type}")
            return result
        return None
    ...
    def generate(self, st):
        ...
        if n_args > 0:
            Code.append(f"  add esp, {n_args * 4} ; limpa {n_args} arg(s)")
...
class Parser:
    ...
    def parse_var_declaration() -> Node:
        ...
        if Parser.lexer.next.type != "TYPE":
            raise Exception("[Parser] Expected type after identifier")
